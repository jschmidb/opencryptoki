 ...
 *
 * Returns the condition code of the PCC instruction
 */
static int s390_pcc(unsigned long func, void *param)
{
    register long r0 __asm__("0") = (unsigned long)func;
    register long r1 __asm__("1") = (unsigned long)param;
    char cc;
...
struct aes_xts_param {
    ...
    unsigned int fc;
    unsigned int keylen;
    // Below: convert_key routine and parms needed by convert_key
    convert_key_t convert_key;
                                        ...
                                        CK_BYTE *iv, void *cb_data)
{
    ...
    while (len2 > 0 && num_tries < PKEY_CONVERT_KEY_RETRIES) {
        ...
        if (bytes_processed < len2) {
            ...
            if (ret != CKR_OK)
                goto done;
            memcpy(param->param_km, protkey, protkey_len);
        }
        in_pos += bytes_processed;
        out_pos += bytes_processed;
